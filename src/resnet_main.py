...
import resnet_models as models
...
from utils.lr_schedulers import scale_initial_learning_rate, MultiStepLRWithWarmup
...
MODEL_NAME = 'resnet50'
MODEL_VERSION = 'v1'
EXPERIMENT_ID = '{}_{}_{}'.format(MODEL_NAME, MODEL_VERSION, datetime.datetime.utcnow().strftime("%Y-%m-%d"))


def define_args():
    parser = argparse.ArgumentParser(description='resnet_main')
    parser.add_argument('--model_name', default=MODEL_NAME, type=str, help='model_name')
    parser.add_argument('--model_version', default=MODEL_VERSION, type=str, help='model_version')
...
def run(cur_gpu, hparams):
    ...
    model = getattr(models, hparams.model_name)(hparams.n_classes, hparams.n_channels, hparams.model_version)
    ...
    lr_scheduler = MultiStepLRWithWarmup(optimizer, hparams.lr_milestones, hparams.lr_warmup_epochs,
...
def train(cur_gpu, train_loader, model, criterion, optimizer, lr_scheduler,
    ...
    for i, (image, target) in enumerate(train_loader):
        ...
        bs = torch.tensor(image.size(0)).cuda(cur_gpu)
