...
from utils.monitors import monitor_context, get_monitor
from utils.checkpoints import load_checkpoint, save_checkpoint


MODEL_NAME = 'anet_small_ws'
...
def set_hparams(hparams):
    ...
    hparams.attended_k = 5
    hparams.attended_epsilon = 0.0
    hparams.emb_dims = 10
...
def main_worker(i, hparams):
    with logger_context(hparams.log_dir, rank=i), \
         monitor_context(hparams.log_dir, hparams.steps_per_epoch, hparams.epochs,
                         hparams.print_freq, hparams.start_epoch, rank=i):
        logger = get_logger()
        if logger:
            logger.log_run_info()
