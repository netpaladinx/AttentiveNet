...
class StageGraph(object):
    def __init__(self, stage_dct):
        ...
        self.n_nodes = stage_dct['n_nodes'] + 2
        ...
        for node in self.sub_inputs:
            ...
            node.in_nodes.append(self.master_input)
            self.master_input.out_nodes.append(node)
            
        for node in self.sub_outputs:
            node.is_sub_output = True
            node.out_nodes.append(self.master_output)
            self.master_output.in_nodes.append(node)

        for e in stage_dct.get('edges', []):
            n0 = self.nodes[e[0]]
            n1 = self.nodes[e[1]]
            n0.out_nodes.append(n1)
            n1.in_nodes.append(n0)
...
class RWGraph(object):
    def __init__(self, name):
        self.name = name
        graph_filepath = self.get_or_build_rwnn_graph(name)
    ...
    def get_or_build_rwnn_graph(self, name):
        ...
        for stage in graph_data['graph']:
            if stage['graph_model'] == 'point':
                stage['inputs'] = [1]
                stage['outputs'] = [1]
            elif stage['graph_model'] == 'erdos_renyi':
                seed = random.randint(a, b)
                rg = RandomGraph('erdos_renyi', stage['n_nodes'], er_p=stage['er_p'], seed=seed)
                stage['seed'] = seed
                stage['inputs'] = rg.inputs
                stage['outputs'] = rg.outputs
                stage['edges'] = rg.dag_edges
            elif stage['graph_model'] == 'barabasi_albert':
                seed = random.randint(a, b)
                rg = RandomGraph('barabasi_albert', stage['n_nodes'], ba_m=stage['ba_m'], seed=seed)
                stage['seed'] = seed
                stage['inputs'] = rg.inputs
                stage['outputs'] = rg.outputs
                stage['edges'] = rg.dag_edges
            elif stage['graph_model'] == 'watts_strogatz':
                seed = random.randint(a, b)
                rg = RandomGraph('watts_strogatz', stage['n_nodes'], ws_k=stage['ws_k'], ws_p=stage['ws_p'], seed=seed)
                stage['seed'] = seed
                stage['inputs'] = rg.inputs
                stage['outputs'] = rg.outputs
                stage['edges'] = rg.dag_edges
            else:
                raise ValueError('Invalid `graph_model`')
    ...
    def draw_rwnn_graph(self, name=None):
        if name is None:
            name = self.name
        data_filepath = os.path.join(GRAPH_DIR, name + '.data.yaml')
...
if __name__ == '__main__':
    rw_graph = RWGraph('rwnn_small_ws_1234')
    rw_graph.draw_rwnn_graph()
